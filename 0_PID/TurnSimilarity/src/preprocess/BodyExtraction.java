	...
	public static ArrayList<Integer> bodyCount(String file) {
		...
		try {
			...
			while ((line = cr.readNext()) != null) {
				if(ids.isEmpty() || !ids.contains(Integer.valueOf(line[18]))) {		// ID is in the 18th column
					ids.add(Integer.valueOf(line[18]));
				}
			}
			cr.close();
		} catch (Exception e) {
	...
	*/
	public static void bodyWriter(String inputfile, String outputfile, int id) {
		...
		try {
			...
			while((line = cr.readNext()) != null) {
				if (Integer.valueOf(line[18]) == id) {			// ID is in the 18th column
					cw.writeNext(line);
				}
        	}
			cw.close();
